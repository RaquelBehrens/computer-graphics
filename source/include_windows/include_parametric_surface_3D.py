...
from objects import (ParametricSurface3D)

class IncludeParametricSurface3D(IncludeWindow):
        ...
        try:
            if self.radio_variable.get() != 0:
                ...
                if name != '' and not already_used:
                    objeto = ParametricSurface3D(name, points, self.color, projection=self.radio_variable.get())
